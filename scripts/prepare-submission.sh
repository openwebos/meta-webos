...
OPTIONS:
  ...
  -s <submission>
        Submission, create submissions/<submission> tag (minimal value $minimal_submission)
...
tag_check_doesnt_exist() {
    ...
    check=`(cd $checkout && git tag -l | grep "^$tag_name$")`
    [ -n "$check" ] && (cd $checkout && git show -1 $tag_name | head) && echo_error "$tag_name tag already exists, refusing to move it!"
}
...
create_tag() {
    tag_name=$1
    (cd $checkout && \
        export cmd="git tag -a -m '$tag_name' $tag_name $NEW_COMMIT" && \
        echo $cmd && \
        $cmd) && \
...
if [ -n "$submission" ]; then
    ...
    if [ "$submission" -ne "$minimal_submission" ]; then
        ...
        if [ -n "$recipe" -a -n "$OLD_SUBMISSION" ] ; then
            LOG=`(cd $checkout && git log --oneline submissions/$OLD_SUBMISSION..$commit)`
        fi
    fi
...
if [ -n "$recipe" ]; then
    ...
    NEW_WEBOS_VERSION_LINE="WEBOS_VERSION ${OLD_WEBOS_VERSION_ASSIGNMENT} \"${VERSION}-${SUBMISSION}_${NEW_COMMIT}\""
