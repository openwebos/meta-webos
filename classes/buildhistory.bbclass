...
#       - buildhistory.bbclass taken from branch=dylan,commit=e174486 of oe-core
#
...
def write_recipehistory(rcpinfo, d):
    ...
    with open(infofile, "w") as f:
        ...
        f.write("DEPENDS = %s\n" %  rcpinfo.depends)
        f.write("PACKAGES = %s\n" %  rcpinfo.packages)
        f.write("SRC_URI = %s\n" % rcpinfo.src_uri)        # WEBOS addition
...
def _webos_append_to_bom(d, srcrevs):
    ...
    data['depends'] = sortlist(squashspaces(d.getVar('DEPENDS', True) or ""))
            ...
            if len(srcrevs) > 1:
                for name, srcrev in srcrevs.items():
                    ...
                    if orig_srcrev:
                        f.write('# SRCREV_%s = "%s"\n' % (name, orig_srcrev))
                    f.write('SRCREV_%s = "%s"\n' % (name, srcrev))
                    bom_srcrevs.append(srcrev)
            else:
                f.write('SRCREV = "%s"\n' % srcrevs.itervalues().next())
                bom_srcrevs.append(srcrevs.itervalues().next())
